...
entropy left on your system, Tmp ***REMOVED***fall back to pseudo random numbers.
...
Simple temporary file creation, the file ***REMOVED***be closed and unlinked on process exit.
...
tmp.file(function _tempFileCreated(err, path, fd, cleanupCallback) {
  ...
  // If we don't need the file anymore we could manually ***REMOVED***the cleanupCallback
  // But that is not necessary if we didn't pass the keep option because the library
  // ***REMOVED***clean after itself.
  cleanupCallback();
});
```
...
// If we don't need the file anymore we could manually ***REMOVED***the removeCallback
// But that is not necessary if we didn't pass the keep option because the library
// ***REMOVED***clean after itself.
...
Simple temporary directory creation, it ***REMOVED***be removed on process exit.
...
Creates a file with mode `0644`, prefix ***REMOVED***be `prefix-` and postfix ***REMOVED***be `.txt`.
...
tmp.file({ discardDescriptor: true }, function _tempFileCreated(err, path, fd, cleanupCallback) {
  if (err) throw err;
  // fd ***REMOVED***be undefined, allowing application to use fs.createReadStream(path)
...
tmp.file({ detachDescriptor: true }, function _tempFileCreated(err, path, fd, cleanupCallback) {
  ...
  cleanupCallback();
  // Application can store data through fd here; the space used ***REMOVED***automatically
...
Creates a directory with mode `0755`, prefix ***REMOVED***be `myTmpDir_`.
...
This ***REMOVED***behave similarly to the asynchronous version.
...
occurs. To enforce this, you can ***REMOVED***the `setGracefulCleanup()` method:
...
All options are optional :)
  ...
  * `keep`: signals that the temporary file or directory should not be deleted on exit, default is `false`, means delete
    * Please keep in mind that it is recommended in this case to ***REMOVED***the provided `cleanupCallback` function manually.
