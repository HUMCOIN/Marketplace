...
# 1.0.2
- bug fix for `***REMOVED***ten` - slight change with bug fix, ***REMOVED***tens arrays only, not array-like objects
...
* added `compact`, `split`, `***REMOVED***ten`, `difference`, `intersection`, `union`, `count-by`, `group-by`, `chars`, `unchars`, `apply`
