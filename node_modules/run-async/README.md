...
Utility method to run a function either synchronously or asynchronously using a series of common patterns. This is useful for library author accepting sync or async functions as parameter. `runAsync` ***REMOVED***always run them as an async method, and normalize the multiple signature.
...
If your version of node support Promises natively (node >= 0.12), `runAsync` ***REMOVED***return a promise. Example: `runAsync(func)(arg1, arg2).then(cb)`
