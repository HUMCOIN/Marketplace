...
Now Prettier ***REMOVED***be run every time the file is saved.
...
To interact with 3rd party plugins, `prettier-vscode` ***REMOVED***look in the project's npm modules, so you'll need to have `prettier` and `prettier-plugin-solidity` in your `package.json`
