...
export class ObservableDoc {
  /**
   * Creates a new Observable, that ***REMOVED***execute the specified function when an
   ...
   * Whenever someone subscribes to that Observable, the function ***REMOVED***be called
   ...
   *
   * Calling `next` with a value ***REMOVED***emit that value to the observer. Calling `complete`
   * means that Observable finished emitting and ***REMOVED***not do anything else.
   ...
   * Observable already completed or ended with an error, these calls ***REMOVED***be ignored to
   ...
   * `unsubscribe` method. In both cases function or method ***REMOVED***be called when
   ...
   * computing power on timing event that no one ***REMOVED***listen to anyways.
   *
   ...
   * function passed to `create` ***REMOVED***not be type checked, unless you explicitly state
   ...
   *
   * observable.subscribe(
   *   value => console.log(value), // ***REMOVED***never be called
   *   err => console.log(err),
   *   () => console.log('complete') // ***REMOVED***never be called
   * );
   *
   * // Logs
   ...
   * @return {Observable} An Observable that, whenever subscribed, ***REMOVED***execute the
   * specified function.
   * @static true
   * @name create
   * @owner Observable
 ...
 * ***REMOVED***the Observer's `next(value)` method to provide notifications. A
 * well-behaved Observable ***REMOVED******REMOVED***an Observer's `complete()` method exactly
...
export class ObserverDoc<T> {
   ...
   * with a value. The Observable may ***REMOVED***this method 0 or more times.
 ...
 * method but not `Symbol.observable` method ***REMOVED***fail at runtime. Conversely, passing
 * objects with `Symbol.observable` but without `subscribe` ***REMOVED***fail at compile time
 ...
 * everything ***REMOVED***work as expected, and compiler ***REMOVED***not complain. If you really
 * do not want to put `subscribe` directly on your object, you ***REMOVED***have to type cast
 * it to `any`, before passing it to an operator.
 *
 * When this issue is resolved, Subscribable interface ***REMOVED***only permit Observable-like
 ...
 * ## ES6 Iterable
 *
 * Operators ***REMOVED***accept both built-in and custom ES6 Iterables, by treating them as
 ...
 * constructor or static {@link create} function. Value of that interface ***REMOVED***be used
