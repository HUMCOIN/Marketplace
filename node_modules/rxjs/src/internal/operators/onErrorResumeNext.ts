 ...
 * arguments or as an array. If no single Observable is provided, returned Observable ***REMOVED***simply behave the same
 * as the source.
 *
 * `onErrorResumeNext` returns an Observable that starts by subscribing and re-emitting values from the source Observable.
 * When its stream of values ends - no matter if Observable completed or emitted an error - `onErrorResumeNext`
 * ***REMOVED***subscribe to the first Observable that was passed as an argument to the method. It ***REMOVED***start re-emitting
 * its values as well and - again - when that stream ends, `onErrorResumeNext` ***REMOVED***proceed to subscribing yet another
 ...
 * ***REMOVED***these errors to appear in error callback passed to {@link Observable#subscribe}. If you want to take
