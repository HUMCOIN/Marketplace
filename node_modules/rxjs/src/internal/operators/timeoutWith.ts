 ...
 * should be an Observable which ***REMOVED***be subscribed when source Observable fails any timeout check.
 * So whenever regular `timeout` would emit an error, `timeoutWith` ***REMOVED***instead start re-emitting
 ...
 * as a consequence - when second Observable ***REMOVED***be subscribed, since subscription happens
 ...
 *   .subscribe(
 *     value => console.log(value), // After 900ms, ***REMOVED***start emitting `minutes`,
 *                                  // since first value of `seconds` ***REMOVED***not arrive fast enough.
 *     err => console.log(err),     // Would be called after 900ms in case of `timeout`,
 *                                  // but here ***REMOVED***never be called.
 *   );
 * ```
 *
 * @param {number|Date} due Number specifying period within which Observable must emit values
 *                          or Date specifying before when Observable should complete
 * @param {Observable<T>} withObservable Observable which ***REMOVED***be subscribed if source fails timeout check.
