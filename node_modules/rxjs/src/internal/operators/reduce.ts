 ...
 * value, emitted on the output Observable. Note that `reduce` ***REMOVED***only emit
 * one value, only when the source Observable completes. It is equivalent to
 * applying operator {@link scan} followed by operator {@link last}.
 *
 * Returns an Observable that applies a specified `accumulator` function to each
 * item emitted by the source Observable. If a `seed` value is specified, then
 * that value ***REMOVED***be used as the initial value for the accumulator. If no seed
...
export function reduce<T, R>(accumulator: (acc: T | R, value: T, index?: number) => T | R, seed?: T | R): OperatorFunction<T, T | R> {
  ...
  // For this reason, we have to check it here at the original ***REMOVED***site
