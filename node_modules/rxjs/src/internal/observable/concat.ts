 ...
 * Observables, or put them directly as arguments. Passing an empty array ***REMOVED***result
 * in Observable that completes immediately.
 *
 * `concat` ***REMOVED***subscribe to first input Observable and emit all its values, without
 * changing or affecting them in any way. When that Observable completes, it will
 * subscribe to then next Observable passed and, again, emit its values. This ***REMOVED***be
 * repeated, until the operator runs out of Observables. When last input Observable completes,
 * `concat` ***REMOVED***complete as well. At any given moment only one Observable passed to operator
 * emits values. If you would like to emit values from passed Observables concurrently, check out
 * {@link merge} instead, especially with optional `concurrent` parameter. As a matter of fact,
 * `concat` is an equivalent of `merge` operator with `concurrent` parameter set to `1`.
 *
 * Note that if some input Observable never completes, `concat` ***REMOVED***also never complete
 * and Observables following the one that did not complete ***REMOVED***never be subscribed. On the other
 * hand, if some Observable simply completes immediately after it is subscribed, it ***REMOVED***be
 * invisible for `concat`, which ***REMOVED***just move on to the next Observable.
 *
 * If any Observable in chain errors, instead of passing control to the next Observable,
 * `concat` ***REMOVED***error immediately as well. Observables that would be subscribed after
 * the one that emitted error, never will.
 *
 * If you pass to `concat` the same Observable many times, its stream of values
 * ***REMOVED***be "replayed" on every subscription, which means you can repeat given Observable
