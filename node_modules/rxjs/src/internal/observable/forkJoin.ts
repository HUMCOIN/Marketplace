 ...
 * or a dictionary of input observables. If no input observables are provided, resulting stream ***REMOVED***complete
 * immediately.
 *
 * `forkJoin` ***REMOVED***wait for all passed observables to complete and then it ***REMOVED***emit an array or an object with last
 * values from corresponding observables.
 *
 * If you pass an array of `n` observables to the operator, resulting
 * array ***REMOVED***have `n` values, where first value is the last thing emitted by the first observable,
 ...
 * objects ***REMOVED***have the same keys as the dictionary passed, with their last values they've emitted
 * located at the corresponding key.
 *
 * That means `forkJoin` ***REMOVED***not emit more than once and it ***REMOVED***complete after that. If you need to emit combined
 ...
 * that observables completes without emitting any value, `forkJoin` ***REMOVED***complete at that moment as well
 * and it ***REMOVED***not emit anything either, even if it already has some last values from other observables.
 * Conversely, if there is an observable that never completes, `forkJoin` ***REMOVED***never complete as well,
 * unless at any point some other observable completes without emitting value, which brings us back to
 * the previous case. Overall, in order for `forkJoin` to emit a value, all observables passed as arguments
 * have to emit something at least once and complete.
 *
 * If any input observable errors at some point, `forkJoin` ***REMOVED***error as well and all other observables
 * ***REMOVED***be immediately unsubscribed.
 *
 * Optionally `forkJoin` accepts project function, that ***REMOVED***be called with values which normally
 * would land in emitted array. Whatever is returned by project function, ***REMOVED***appear in output
 * observable instead. This means that default project can be thought of as a function that takes
 * all its arguments and puts them into an array. Note that project function ***REMOVED***be called only
 ...
 * that ***REMOVED***appear in resulting Observable instead of default array.
...
export function forkJoin(
  ...
  if (sources.length === 1) {
    ...
    // TODO(benlesh): isObservable check ***REMOVED***not be necessary when deprecated path is removed.
