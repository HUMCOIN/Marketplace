 ...
 * returned Observable ***REMOVED***always emit an array of `n` values, in order
 ...
 * you ***REMOVED***combine. Passing empty array ***REMOVED***result in Observable that
 ...
 * ***REMOVED***be lost. On the other hand, if some Observable does not emit a value but
 * completes, resulting Observable ***REMOVED***complete at the same moment without
 * emitting anything, since it ***REMOVED***be now impossible to include value from
 * completed Observable in resulting array. Also, if some input Observable does
 * not emit any value and never completes, `combineLatest` ***REMOVED***also never emit
 * and never complete, since, again, it ***REMOVED***wait for all streams to emit some
 * value.
 *
 * If at least one Observable was passed to `combineLatest` and all passed Observables
 * emitted something, resulting Observable ***REMOVED***complete when all combined
 * streams complete. So even if some Observable completes, result of
 * `combineLatest` ***REMOVED***still emit values when other Observables do. In case
 * of completed Observable, its value from now on ***REMOVED***always be the last
 * emitted value. On the other hand, if any Observable errors, `combineLatest`
 * ***REMOVED***error immediately as well, and all other Observables ***REMOVED***be unsubscribed.
 *
 * `combineLatest` accepts as optional parameter `project` function, which takes
 * as arguments all values that would normally be emitted by resulting Observable.
 * `project` can return any kind of value, which ***REMOVED***be then emitted by Observable
