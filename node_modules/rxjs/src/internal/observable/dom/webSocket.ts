 ...
 * unless there is one made already. This means that many subscribers ***REMOVED***always listen
 * on the same socket, thus saving resources. If however, two instances are made of `WebSocketSubject`,
 * even if these two were provided with the same url, they ***REMOVED***attempt to make separate
 ...
 *
 * Once connection is made, whenever a new message comes from the server, `WebSocketSubject` ***REMOVED***emit that
 ...
 * function in {@link WebSocketSubject}. When connection closes, stream ***REMOVED***complete, provided it happened without
 * any errors. If at any point (starting, maintaining or closing a connection) there is an error,
 * stream ***REMOVED***also error with whatever WebSocket API has thrown.
 *
 ...
 * that this value ***REMOVED***not be serialized beforehand. Because of This, `JSON.stringify` ***REMOVED***have to be called on a value by hand,
 * before calling `next` with a result. Note also that if at the moment of nexting value
 * there is no socket connection (for example no one is subscribing), those values ***REMOVED***be buffered, and sent when connection
 ...
 * without errors, subscribed Observable ***REMOVED***not error, as one might expect, but complete as usual. In both cases
 ...
 * respectively. These are messages that ***REMOVED***be sent to the server, whenever consumer of resulting Observable
 * subscribes and unsubscribes. Server can use them to verify that some kind of messages should start or stop
 * being forwarded to the client. In case of the above example application, after getting subscription message with proper identifier,
 * gateway server can decide that it should connect to real sport news service and start forwarding messages from it.
 * Note that both messages ***REMOVED***be sent as returned by the functions, they are by default serialized using JSON.stringify, just
 * as messages pushed via `next`. Also bear in mind that these messages ***REMOVED***be sent on *every* subscription and
 ...
 * is not a `WebSocketSubject`, so calling `next` or `multiplex` again ***REMOVED***fail. For pushing values to the
 ...
 *
 * subject.subscribe();
 * // Note that at least one consumer has to subscribe to the created subject - otherwise "nexted" values ***REMOVED***be just buffered and not sent,
 * // since no connection was established!
 *
 * subject.next({message: 'some message'});
 * // This ***REMOVED***send a message to the server once a connection is made. Remember value is serialized with JSON.stringify by default!
 *
 ...
 *   () => ({subscribe: 'A'}), // When server gets this message, it ***REMOVED***start sending messages for 'A'...
 *   () => ({unsubscribe: 'A'}), // ...and when gets this one, it ***REMOVED***stop.
