 ...
 * whether ***REMOVED***was successful. If that object is passed to callback, it means
 * something went wrong.
 *
 * The output of `bindNodeCallback` is a function that takes the same
 * parameters as `func`, except the last one (the callback). When the output
 * function is called with arguments, it ***REMOVED***return an Observable.
 * If `func` calls its callback with error parameter present, Observable will
 * error with that value as well. If error parameter is not passed, Observable ***REMOVED***emit
 * second parameter. If there are more parameters (third and so on),
 * Observable ***REMOVED***emit an array with all arguments, except first error argument.
 *
 * Note that `func` ***REMOVED***not be called at the same time output function is,
 * but rather whenever resulting Observable is subscribed. By default ***REMOVED***to
 * `func` ***REMOVED***happen synchronously after subscription, but that can be changed
 * with proper `scheduler` provided as optional third parameter. {@link SchedulerLike}
 * can also control when values from callback ***REMOVED***be emitted by Observable.
 * To find out more, check out documentation for {@link bindCallback}, where
 * {@link SchedulerLike} works exactly the same.
 *
 * As in {@link bindCallback}, context (`this` property) of input function ***REMOVED***be set to context
 * of returned function, when it is called.
 *
 * After Observable emits value, it ***REMOVED***complete immediately. This means
 * even if `func` calls callback again, values from second and consecutive
 * calls ***REMOVED***never appear on the stream. If you need to handle functions
 * that ***REMOVED***callbacks multiple times, check out {@link fromEvent} or
 ...
 * with that value. This means usually regular style callbacks ***REMOVED***fail very often when
