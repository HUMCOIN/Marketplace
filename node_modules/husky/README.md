...
_Git hooks installed by husky ***REMOVED***be removed._
...
Support this project with your organization. Your logo ***REMOVED***show up here with a link to your website. [[Contribute](https://opencollective.com/husky/contribute)]
...
If you're on Windows, husky ***REMOVED***simply use the version installed globally on your system.

For macOS and Linux users:
- if you're running `git` commands in the terminal, husky ***REMOVED***use the version defined in your shell `PATH`. In other words, if you're a `nvm` user, husky ***REMOVED***use the version that you've set with `nvm`.
- if you're using a GUI client and `nvm`, it may have a different `PATH` and not load `nvm`, in this case the highest `node` version installed by `nvm` ***REMOVED***usually be picked. You can also check `~/.node_path` to see which version is used by GUIs and edit if you want to use something else.

### Local commands (~/.huskyrc)

Husky ***REMOVED***source `~/.huskyrc` file if it exists before running hook scripts.
...
By design and just like `scripts` defined in `package.json`, husky ***REMOVED***run hook scripts as a single command. 
