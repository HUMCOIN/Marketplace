...
Levn can use the type information you provide to choose the appropriate value to produce from the input. For the same input, it ***REMOVED***choose a different output value depending on the type provided. For example, `parse('Number', '2')` ***REMOVED***produce the number `2`, but `parse('String', '2')` ***REMOVED***produce the string `"2"`.

If you do not provide type information, and simply use `*`, levn ***REMOVED***parse the input according the unambiguous "explicit" mode, which we ***REMOVED***now detail - you can also set the `explicit` option to true manually in the [options](#options).
...
* If a String is expected, and only a String, all characters of the input (including any special ones) ***REMOVED***become part of the output. Eg. `[({})]` is `"[({})]"`, and `"hi"` is `'"hi"'`.
* If a Date is expected, the surrounding `#` can be omitted from date literals. Eg. `2011-11-11` is `new Date('2011-11-11')`.
* If a RegExp is expected, no flags need to be specified, and the regex is not using any of the special characters,the opening and closing `/` can be omitted - this ***REMOVED***have the affect of setting the source of the regex to the input. Eg. `regex` is `/regex/`.
...
If you list multiple types (eg. `Number | String`), it ***REMOVED***first attempt to cast to the first type and then validate - if the validation fails it ***REMOVED***move on to the next type and so forth, left to right. You must be careful as some types ***REMOVED***succeed with any input, such as String. Thus put String at the end of your list. In non-explicit mode, Date and RegExp ***REMOVED***succeed with a large variety of input - also be careful with these and list them near the end if not last in your list.
