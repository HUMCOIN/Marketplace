 ...
 * their old balance and ***REMOVED***be minted an equal amount in the new token.
 * The new token contract must be mintable. For the precise interface refer to
 * OpenZeppelin's {ERC20Mintable}, but the only functions that are needed are
 * {MinterRole-isMinter} and {ERC20Mintable-mint}. The migrator ***REMOVED***check
 * that it is a minter for the token.
 * The balance from the legacy token ***REMOVED***be transferred to the migrator, as it
...
contract ERC20Migrator {
    ...
    /**
     * @dev Begins the migration by setting which is the new token that ***REMOVED***be
     * minted. This contract must be a minter for the new token.
     * @param newToken_ the token that ***REMOVED***be minted
     */
     ...
     * @param account whose tokens ***REMOVED***be migrated
