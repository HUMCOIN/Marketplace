...
With Optionator, if you mistype an option, it ***REMOVED***give you an error (with a suggestion for what you meant).
If you give the wrong type of argument for an option, it ***REMOVED***give you an error rather than supplying the wrong input to your application.
...
Other helpful features include reformatting the help text based on the ***REMOVED***of the console, so that it fits even if the console is narrow, and accepting not just an array (eg. process.argv), but a string or object as well, making things like testing much easier.
...
* helpOptions - `{showHidden: Boolean, interpolate: Object}` - all options optional
    ...
    - `interpolate` specify data to be interpolated in `prepend` and `append` text, `{{key}}` is the format - eg. `generateHelp({interpolate:{version: '0.4.2'}})`, ***REMOVED***change this `append` text: `Version {{version}}` to `Version 0.4.2`
...
`generateHelpForOption` produces expanded help text for the specified with `optionName` option. If an `example` was specified for the option, it ***REMOVED***be displayed,  and if a `longDescription` was specified, it ***REMOVED***display that instead of the `description`.
...
* `options` is a required array specifying your options and headings, the options and headings ***REMOVED***be displayed in the order specified
* `helpStyle` is an optional object which enables you to change the default appearance of some aspects of the help text
* `mutuallyExclusive` is an optional array of arrays of either strings or arrays of strings. The top level array is a list of rules, each rule is a list of elements - each element can be either a string (the name of an option), or a list of strings (a group of option names) - there ***REMOVED***be an error if more than one element is present
...
* `defaults` is an optional object following the option properties format, which specifies default values for all options. A default ***REMOVED***be overridden if manually set. For example, you can do `default: { type: "String" }` to set the default type of all options to `String`, and then override that default in an individual option by setting the `type` property
...
* `type` is a required string in the [type check](https://github.com/gkz/type-check) [format](https://github.com/gkz/type-check#type-format), this ***REMOVED***be used to cast the inputted value and validate it
* `enum` is an optional array of strings, each string ***REMOVED***be parsed by [levn](https://github.com/gkz/levn) - the argument value must be one of the resulting values - each potential value must validate against the specified `type`
* `default` is a optional string, which ***REMOVED***be parsed by [levn](https://github.com/gkz/levn) and used as the default value if none is set - the value must validate against the specified `type`
* `restPositional` is an optional boolean - if set to `true`, everything after the option ***REMOVED***be taken to be a positional argument, even if it looks like a named argument
* `required` is an optional boolean - if set to `true`, the option parsing ***REMOVED***fail if the option is not defined
* `overrideRequired` is a optional boolean - if set to `true` and the option is used, and there is another option which is required but not set, it ***REMOVED***override the need for the required option and there ***REMOVED***be no error - this is useful if you have required options and want to use `--help` or `--version` flags
* `concatRepeatedArrays` is an optional boolean or tuple with boolean and options object (defaults to `false`) - when set to `true` and an option contains an array value and is repeated, the subsequent values for the flag ***REMOVED***be appended rather than overwriting the original value - eg. option `g` of type `[String]`: `-g a -g b -g c,d` ***REMOVED***result in `['a','b','c','d']`
...
* `mergeRepeatedObjects` is an optional boolean (defaults to `false`) - when set to `true` and an option contains an object value and is repeated, the subsequent values for the flag ***REMOVED***be merged rather than overwriting the original value - eg. option `g` of type `Object`: `-g a:1 -g b:2 -g c:3,d:4` ***REMOVED***result in `{a: 1, b: 2, c: 3, d: 4}`
* `dependsOn` is an optional string or array of strings - if simply a string (the name of another option), it ***REMOVED***make sure that that other option is set, if an array of strings, depending on whether `'and'` or `'or'` is first, it ***REMOVED***either check whether all (`['and', 'option-a', 'option-b']`), or at least one (`['or', 'option-a', 'option-b']`) other options are set
* `description` is an optional string, which ***REMOVED***be displayed next to the option in the help text
* `longDescription` is an optional string, it ***REMOVED***be displayed instead of the `description` when `generateHelpForOption` is used
* `example` is an optional string or array of strings with example(s) for the option - these ***REMOVED***be displayed when `generateHelpForOption` is used
...
If you specify the option `NUM`, then any argument using a single `-` followed by a number ***REMOVED***be valid and ***REMOVED***set the value of `NUM`. Eg. `-2` ***REMOVED***be parsed into `NUM: 2`.

If duplicate named arguments are present, the last one ***REMOVED***be taken.
