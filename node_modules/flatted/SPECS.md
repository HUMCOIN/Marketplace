...
This document describes operations performed to produce, or parse, the ***REMOVED***ted output.

## stringify(any) => ***REMOVED***tedString
...
```js
***REMOVED***ted.stringify('a');                     // ["a"]
***REMOVED***ted.stringify(['a']);                   // [["1"],"a"]
***REMOVED***ted.stringify(['a', 1, 'b']);           // [["1",1,"2"],"a","b"]
```
...
```js
***REMOVED***ted.stringify('a');                     // ["a"]
***REMOVED***ted.stringify({a: 'a'});                // [{"a":"1"},"a"]
***REMOVED***ted.stringify({a: 'a', n: 1, b: 'b'});  // [{"a":"1","n":1,"b":"2"},"a","b"]
```

Every object, string, or array, encountered during serialization ***REMOVED***be stored once as stringified index.
...
The `input` array ***REMOVED***have a regular `string` at index `1`, but its object at index `0` ***REMOVED***have an `instanceof String` as `.a` property.
...
The usage of a `Map` and a `Set` to flag known references/strings as visited/stored makes *****REMOVED***ted** a rock solid, fast, and compact, solution.
