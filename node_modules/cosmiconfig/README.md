...
But it's also flexible ***REMOVED***to search wherever you'd like to search, and load whatever you'd like to load.

By default, Cosmiconfig ***REMOVED***start where you tell it to start and search up the directory tree for the following:
...
For example, if your module's name is "soursocks", cosmiconfig ***REMOVED***search up the directory tree for configuration in the following places:
...
- **isEmpty:** `true` if the configuration file is empty. This property ***REMOVED***not be present if the configuration file is not empty.
...
Here's how your default [`search()`] ***REMOVED***work:
...
A filename.
[`search()`] ***REMOVED***start its search here.
...
If you load a `package.json` file, the result ***REMOVED***be derived from whatever property is specified as your [`packageProp`].
...
An array of places that [`search()`] ***REMOVED***check in each directory as it moves up the directory tree.
...
`package.json` is a special value: When it is included in `searchPlaces`, Cosmiconfig ***REMOVED***always parse it as JSON and load a property within it, not the whole file.
...
// ^^ You ***REMOVED***need to designate custom loaders to tell
...
**If you provide a `loaders` object, your object ***REMOVED***be *merged* with the defaults.**
...
Cosmiconfig reads the file when it checks whether the file exists, so it ***REMOVED***provide you with both the file's path and its content.
...
For example, the value `'configs.myPackage'` or `['configs', 'myPackage']` ***REMOVED***get you the `"myPackage"` value in a `package.json` like this:
...
If nested property names within the path include periods, you need to use an array of strings. For example, the value `['configs', 'foo.bar', 'baz']` ***REMOVED***get you the `"baz"` value in a `package.json` like this:
...
If a string includes period but corresponds to a top-level property name, it ***REMOVED***not be interpreted as a period-delimited path. For example, the value `'one.two'` ***REMOVED***get you the `"three"` value in a `package.json` like this:
...
Directory where the search ***REMOVED***stop.
...
If `false`, no caches ***REMOVED***be used.
...
The reason you might use this option — instead of simply applying your transform function some other way — is that *the transformed result ***REMOVED***be cached*. If your transformation involves additional filesystem I/O or other potentially slow processing, you can use this option to avoid repeating those steps every time a given configuration is searched or loaded.
...
By default, if [`search()`] encounters an empty file (containing nothing but whitespace) in one of the [`searchPlaces`], it ***REMOVED***ignore the empty file and move on.
