...
- Changed: error message format for `JSON` parse errors(see [#101][pr-101]). If you were relying on the format of JSON-parsing error messages, this ***REMOVED***be a breaking change for you.
...
- Removed: support for loading config path using the `--config` flag. cosmiconfig ***REMOVED***not parse command line arguments. Your application can parse command line arguments and pass them to cosmiconfig.
